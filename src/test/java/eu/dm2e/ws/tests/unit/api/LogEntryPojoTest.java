...
public class LogEntryPojoTest extends OmnomUnitTest{
	...
	public void testSerializeJsonNoId() {
		...
		assertEquals(testGson.toJson(expected), GrafeoJsonSerializer.serializeToJSON(logE, LogEntryPojo.class));
	}
...
//		expected.addProperty(SerializablePojo.JSON_FIELD_UUID, logE.getUuid());
	...
	public void testDeserializeJsonWithOutId() {
		...
		LogEntryPojo logE = GrafeoJsonSerializer.deserializeFromJSON(asJson.toString(), LogEntryPojo.class);
		...
		GrafeoAssert.graphsAreEquivalent(expected.getGrafeo(), logE.getGrafeo());
