...
//		log.debug("" + invoc.);
		Response resp = invoc.invoke();
		if (resp.getStatus() == Response.Status.TEMPORARY_REDIRECT.getStatusCode()) {
			invoc = client.target(resp.getLocation()).request().build("GET");
		}
	...
	private boolean isMetadataKept(String fileUri) {
		...
		fp.setFileRetrievalURI((URI)null);
		fp.setInternalFileLocation(null);
		fp.setCreated(null);
		...
		GrafeoAssert.graphContainsGraphStructurally(FILE_POJO.getGrafeo(), fp.getGrafeo());
...
//		}
	...
	public void setUp() throws Exception {
		...
		FILE_POJO.setFileOwner(new UserPojo());
//		FILE_POJO.setFileRetrievalURI("http://dummy");
		FILE_POJO.setFileType("someType");
		FILE_POJO.setExtent(13L);
		FILE_POJO.setFormat("application/octet-stream");
