...
//		Gson gson = new GsonBuilder().create();
		...
		ParameterPojo p2 = OmnomJsonSerializer.deserializeFromJSON(p.toJson(), ParameterPojo.class);
		GrafeoAssert.graphsAreEquivalent(p, p2);
...
//		Gson gson = new GsonBuilder().create();
		...
		WorkflowPositionPojo p2 = OmnomJsonSerializer.deserializeFromJSON(p.toJson(), WorkflowPositionPojo.class);
		GrafeoAssert.graphsAreEquivalent(p, p2);
		log.debug(p2.toJson());
	}
...
//		System.in.read();
		WorkflowPojo x = OmnomJsonSerializer.deserializeFromJSON(wf.toJson(), WorkflowPojo.class);
		...
		GrafeoAssert.graphsAreEquivalent(wf, wfCopy);
		
		{
			WorkflowPojo wfParsed = OmnomJsonSerializer.deserializeFromJSON(wfCopy.toJson(), WorkflowPojo.class);
			...
			long expectedSize = wf.getGrafeo().size()
					...
					;
			GrafeoAssert.graphsAreEquivalent(wf, wfParsed);
			GrafeoAssert.sizeEquals(wfParsed, wfParsed);
